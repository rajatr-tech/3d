...
func _physics_process(delta: float) -> void:
	...
	if is_on_wall() and turning==false:
		turn_around()
	if not $RayCast3D.is_colliding() and is_on_floor() and turning==false and turns_around_at_edges:
		turn_around()
...
func _on_sidechecker_body_entered(body: Node3D) -> void:
	SoundManager.play_enemy_sound()
	get_tree().change_scene_to_file("res://menu_game_over.tscn")


func _on_topchecker_body_entered(body: Node3D) -> void:
	$Soundsquash.play()
